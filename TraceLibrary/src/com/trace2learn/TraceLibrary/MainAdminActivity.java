...
public class MainAdminActivity extends TraceListActivity {
	...
	static final String[] APPS = new String[] { "Create Character",
			"Create Phrase", "Browse All Characters", "Browse All Phrases",
			"Browse All Collections", "Export To File", "Import From File",
			"Export SQLite Database", "Integrity Check" };
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (clicked.equals(APPS[0])) { // "Create Character"
					Intent i = new Intent().setClass(c,
							ViewCharacterActivity.class);
					startActivity(i);
	...
	private void exportDatabase() {
		try {
			...
			// Open the .db file
			String dbPath = getDatabasePath(DbAdapter.DATABASE_NAME)
					.getAbsolutePath();
			...
			String outPath = Environment.getExternalStorageDirectory()
					.getAbsolutePath()
					+ "/data/"
					+ getString(R.string.file_dir_name);
			...
			Log.e("Export DB", e.getClass().getName() + ": " + e.getMessage());
			e.printStackTrace();
		}
	}

	private void integrityCheck() {
		// - flag characters without id or pinyin tag
		// - flag characters without free-form labels
		// - flag duplicate pinyin tags
		// - flag phrases without pinyin tag or free-form labels
		...
		for (LessonItem item : chars) {
			...
			if (ch.getTagsToString().length() ==0)
				msg = msg + "Missing tags for " + ch.getKeyValuesToString() + "\n";
		...
		for (String wid : wids) {
			...
			if (!wd.hasKey(Toolbox.PINYIN_KEY))
				msg = msg + "Missing pinyin for phrase "
						...
						+ wd.getTagsToString() + "\n";
			if(wd.getTagsToString().length() == 0)
				msg = msg + "Missing tags for phrase "
						+ Toolbox.dba.getKeyValues(wid, ItemType.WORD) + "\n";
