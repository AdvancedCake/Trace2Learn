...
public class PhrasePracticeActivity extends TraceBaseActivity {
	...
	private void getViews() {
		...
		tagLabel		= (TextView) findViewById(R.id.tag_label);
		...
		//tagScroll		= (HorizontalScrollView) findViewById(R.id.tag_scroll_view);
	...
	private void getHandlers() {
		...
		tagLabel.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				toggleTagView(false);
			}
		...
		charSlot.setOnTouchListener(new OnSwipeTouchListener(this
	...
	private void initializeMode() {
		...
		if (bun != null && bun.containsKey("wordId")) {
			...
			if (lessonID == null) {
				...
				lessonName = Toolbox.dba.getLessonById(lessonID)
						.getLessonName();
			...
			if (word.hasKey(Toolbox.SOUND_KEY)
					|| word.hasKey(Toolbox.PINYIN_KEY)) {
				soundPool = new SoundPool(1, AudioManager.STREAM_RING, 0);
				// use 'sound' tag to locate audio if it exists, otherwise use
				// 'pinyin' tag
				String audioKey = word.hasKey(Toolbox.SOUND_KEY) ? Toolbox.SOUND_KEY
						: Toolbox.PINYIN_KEY;
	...
	private void setSelectedCharacter(int position, Scroll scrollage) {
		...
		// scroll the gallery, if necessary, to ensure that new selection is
		// visible
		...
		if (scrollage == Scroll.LEFT && selectionLeftEdge < visibleLeftEdge)
			thumbScroll.scrollBy(thumbWidth * (-1), 0);
		if (scrollage == Scroll.RIGHT && selectionRightEdge > visibleRightEdge)
	...
	private void setCharacterList(List<String> ids) {
		...
		for (String id : ids) {
			...
			// duration of rendering is dynamic based on # of strokes, 2 strokes
			// per second
			int duration = ch.getNumStrokes() / 2 + 1;

			CharacterPlaybackPane dispPane = new CharacterPlaybackPane(context,
					false, duration);
	...
	private void updateTags() {
		if (word != null) {
			// display any tags
			String label = word.getTagsToString();

			// display the pinyin value, if it exists
			if (word.hasKey(Toolbox.PINYIN_KEY)) {
				String pinyin = "(" + word.getValue(Toolbox.PINYIN_KEY) + ")";
				label += "\n";
				label += pinyin;
			}
			...
			if (currentChar + 1 < characters.size()) {
				...
				if (lessonID != null) {
					if (phraseIndex < collectionSize) { // still more words
						...
						Toolbox.showToast(getApplicationContext(), "Completed "
								+ lessonName);
					}
				}
			}
